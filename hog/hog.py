...
def roll_dice(num_rolls, dice=six_sided):
    ...
    "*** YOUR CODE HERE ***"
    sum_val = 0
    ss_cond = False # Sow Sad Condition
    while (num_rolls != 0):
        throw_val = dice()
        if throw_val == 1: 
            ss_cond = True
...
def hefty_hogs(player_score, opponent_score):
    ...
    def func(n, net_val):
        if (n < 10):
            cur_dig_func = digit_fn(n)
            net_val = cur_dig_func(net_val)
            return net_val
        else:
            cur_dig_func = digit_fn(n % 10)
            net_val = cur_dig_func(net_val)
            return func(n // 10, net_val)
    result = func(opponent_score, player_score)
    return (result % 30)
    # END PROBLEM 2
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while (score0 < goal and score1 < goal):
        cur_strategy = strategy0(score0, score1) if (who == 0) else strategy1(score1, score0)
        cur_score = score0 if (who == 0) else score1
        opp_score = score1 if (who == 0) else score0

        cur_score += take_turn(cur_strategy, cur_score, opp_score, dice = dice_in, goal = goal_in)
        # implementing Hog Pile
        cur_score += hog_pile(cur_score, opp_score)
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if (score0 > score1):
        leader = 0
        message = str("Player ") + str(int(leader)) + " takes the lead by " + str(int(score0 - score1))
    elif (score0 == score1):
        leader = None
        message = None
    else:
        leader = 1
        message = str("Player ") + str(int(leader)) + " takes the lead by " + str(int(score1 - score0))
...
def make_averaged(original_function, total_samples=1000):
    ...
    def dice_in(*args):
        ...
        for i in range(0,total_samples):
            valid_dice_in = original_function(*args)
            sum += valid_dice_in
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    avg_dict = dict()
    for i in range(1, 11):
        avg_dict[i] = make_averaged(roll_dice,total_samples)(i, dice)
    max = 1 # putting the first index as the max index
    for i in range(1,11):
        if (avg_dict[i] > avg_dict[max]):
            max = i
    return max
    # END PROBLEM 9
...
def hefty_hogs_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    hefty_hogs_return = hefty_hogs(score, opponent_score)
    if (hefty_hogs_return >= threshold):
        return 0
    else:
        return num_rolls
    # END PROBLEM 10
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    # My Code
    hefty_hogs_strategy_return = hefty_hogs_strategy(score, opponent_score, threshold, num_rolls)
    if (hog_pile(score + hefty_hogs(score, opponent_score), opponent_score) != 0):
        return 0
    elif (hefty_hogs_strategy_return == 0):
        return 0
    else:
        return num_rolls
    """
    if (hefty_hogs_strategy_return == 0):
        if ((hefty_hogs(score, opponent_score) + score), opponent_score >= threshold):
            return 0
        else:
            return num_rolls
    else:
